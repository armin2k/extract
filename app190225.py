...
<div class="container">
  ...
  <h2>Balance Sheet Detail for {{ sheet.company_name }} ({{ sheet.cnpj }})</h2>
...
def upload():
    try:
        ...
        # Join the list of cleaned pages into a single string for wrapping.
        wrapped_json = wrap_pages_in_json("\n".join(cleaned_pages))
        ...
        # 5. Save the analysis result to the database (prevent duplicates)
        ...
        if existing_sheet:
            ...
            db.commit()
            record = existing_sheet
            logger.info("Updated existing record for company %s.", company_name)
        else:
            record = BalanceSheet(
                filename=filename,
...
def view_sheet(sheet_id: int):
    ...
    if sheet:
        try:
            ...
            data = {}
        return render_template_string(VIEW_HTML, sheet=sheet, data=data)
    else:
